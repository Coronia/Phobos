	...
	if (shieldDamage > 0)
	{
		...
		if (residueDamage >= 0)
		{
			...
			if (pSource)
			{
				...
				const bool allFire = pWHExt->Shield_RevengeWeapon_AllFire.Get(this->Type->RevengeWeapon_AllFire);
				bool hasFilters = pWHExt->SuppressRevengeWeapons_Types.size() > 0;

				if (pWeaponType && EnumFunctions::CanTargetHouse(affectedHouses, pTechno->Owner, pSource->Owner))
				{
					if (!pWHExt->SuppressRevengeWeapons || (hasFilters && !pWHExt->SuppressRevengeWeapons_Types.Contains(pWeaponType)))
						WeaponTypeExt::DetonateAt(pWeaponType, pSource, pTechno);
				}
				...
				if (pTypeExt && pTypeExt->RevengeWeapon_FireAtShieldBreak.Get(allFire) && pTypeExt->RevengeWeapon
					&& EnumFunctions::CanTargetHouse(pTypeExt->RevengeWeapon_AffectsHouses, pTechno->Owner, pSource->Owner))
				{
					if (!pWHExt->SuppressRevengeWeapons || (hasFilters && !pWHExt->SuppressRevengeWeapons_Types.Contains(pTypeExt->RevengeWeapon)))
						WeaponTypeExt::DetonateAt(pTypeExt->RevengeWeapon, pSource, pTechno);
				}
				...
				for (auto& attachEffect : pExt->AttachedEffects)
				{
					...
					if (!pType->RevengeWeapon_FireAtShieldBreak.Get(allFire))
						continue;
					...
					if (EnumFunctions::CanTargetHouse(pType->RevengeWeapon_AffectsHouses, pTechno->Owner, pSource->Owner))
						WeaponTypeExt::DetonateAt(pType->RevengeWeapon, pSource, pTechno);
				}
			}
