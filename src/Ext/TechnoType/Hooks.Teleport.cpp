...
DEFINE_HOOK(0x7193F6, TeleportLocomotionClass_ILocomotion_Process_WarpoutAnim, 0x6)
{
	...
	{
		if (const auto pAnim = GameCreate<AnimClass>(pWarpOut, pLinked->Location))
...
DEFINE_HOOK(0x719742, TeleportLocomotionClass_ILocomotion_Process_WarpInAnim, 0x5)
{
	...
	{
		if (const auto pAnim = GameCreate<AnimClass>(pWarpIn, pLinked->Location))
			AnimExt::SetAnimOwnerHouseKind(pAnim, pLinked->Owner, nullptr, false, true);
	}
...
DEFINE_HOOK(0x719827, TeleportLocomotionClass_ILocomotion_Process_WarpAway, 0x5)
{
	...
	{
		if (const auto pAnim = GameCreate<AnimClass>(pWarpAway, pLinked->Location))
			AnimExt::SetAnimOwnerHouseKind(pAnim, pLinked->Owner, nullptr, false, true);
	}
