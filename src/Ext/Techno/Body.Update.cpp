...
void TechnoExt::ExtData::UpdateTypeData(TechnoTypeClass* pCurrentType)
{
	...
	for (auto const& entry : this->TypeExtData->LaserTrailData)
	{
		...
		{
			this->LaserTrails.emplace_back(std::make_unique<LaserTrailClass>(
...
void TechnoExt::ExtData::UpdateLaserTrails()
{
	...
	for (auto const& trail : this->LaserTrails)
	{
		...
		trail->Cloaked = false;
		const bool cloakVisible = pType->CloakVisible;

		if (pThis->CloakState == CloakState::Cloaked)
		{
			if (cloakVisible && pType->CloakVisible_DetectedOnly && !HouseClass::IsCurrentPlayerObserver() && !pThis->Owner->IsAlliedWith(HouseClass::CurrentPlayer))
				trail->Cloaked = !pThis->GetCell()->Sensors_InclHouse(HouseClass::CurrentPlayer->ArrayIndex);
			else if (!cloakVisible)
				trail->Cloaked = true;
		}
		...
		if (pThis->CloakState == CloakState::Uncloaking && !cloakVisible)
